...
namespace SmppClient.Core
{
    ...
    public class DataSmResp : Header, IPacket, IPduDetails
    {
        ...
        public byte[] GetPDU()
        {
            ...
            if (Optional.Count > 0) tmpBuff.AddTLVCollection(Optional);
        ...
        public List<PduPropertyDetail> Details()
        {
            ...
            try
            {
                ...
                if (details[0].ValueUInt > HEADER_LENGTH)
                {
        ...
        private DataSmResp(DataCodings defaultEncoding) : base(defaultEncoding,
            CommandSet.DataSmResp)
        {
        ...
        private DataSmResp(DataCodings defaultEncoding,
        ...
        public static DataSmResp Create(DataCodings defaultEncoding,
            ...
            string messageId)
        {
            if (dataSm.SourceAddr == string.Empty || dataSm.SourceAddr == null) commandStatus = CommandStatus.ESME_RINVSRCADR;
        ...
        public static DataSmResp Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var dataSmResp = new DataSmResp(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(dataSmResp,
                    ref offset);

                if (dataSmResp.Length > HEADER_LENGTH)
                {
                    dataSmResp.MessageId = buf.ExtractCString(ref offset);

                    while (offset - start_offset < dataSmResp.Length) dataSmResp.Optional.Add(buf.ExtractTLV(ref offset));
                }
            }

            catch { dataSmResp = null; }
