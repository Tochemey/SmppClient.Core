...
{
    /// <summary>
    ///     The deliver_sm is issued by the SMSC (Short Message Service Centre) to send a
    ///     message to an ESME (Extended Short Message Entity). Using this command, the SMSC may route a
    ///     short message to the ESME for delivery.
    /// </summary>
    public class DeliverSm : Header, IPacket, IPduDetails
    {
        ...
        public byte[] GetPDU()
        {
            if (UserData.Headers.Count > 0) EsmClass |= 0x40;
            ...
            tmpBuff.AddTimeString(ScheduleDeliveryTime,
                17);
            tmpBuff.AddTimeString(ValidityPeriod,
                17);
        ...
        public List<PduPropertyDetail> Details()
        {
            ...
            try
            {
                ...
                details.Add(PduData.ExtractCString("ServiceType",
                    ref offset));
                details.Add(PduData.ExtractByte("SourceTon",
                    ref offset));
                details.Add(PduData.ExtractByte("SourceNpi",
                    ref offset));
                details.Add(PduData.ExtractCString("SourceAddr",
                    ref offset));
                details.Add(PduData.ExtractByte("DestTon",
                    ref offset));
                details.Add(PduData.ExtractByte("DestNpi",
                    ref offset));
                details.Add(PduData.ExtractCString("DestAddr",
                    ref offset));

                var esmClass = PduData.ExtractByte("EsmClass",
                    ref offset);
                details.Add(esmClass);

                details.Add(PduData.ExtractByte("ProtocolId",
                    ref offset));
                details.Add(PduData.ExtractByte("PriorityFlag",
                    ref offset));
                details.Add(PduData.ExtractCString("ScheduleDeliveryTime",
                    ref offset));
                details.Add(PduData.ExtractCString("ValidityPeriod",
                    ref offset));
                details.Add(PduData.ExtractByte("RegisteredDelivery",
                    ref offset));
                details.Add(PduData.ExtractByte("ReplaceIfPresent",
                    ref offset));

                var dataCoding = PduData.ExtractByte("DataCoding",
                    ref offset);
                details.Add(dataCoding);

                details.Add(PduData.ExtractByte("DefaultMessageId",
                    ref offset));

                var messageLength = PduData.ExtractByte("ShortMessageLength",
                    ref offset);
                details.Add(messageLength);

                if (messageLength.ValueByte > 0)
                {
                    var userDataProperty = PduData.ExtractByteArray("ShortMessage",
                        ref offset,
                        messageLength.ValueByte);
                    userDataProperty.PduDataType = PduDataTypes.EncodedString;

                    var userData = UserData.Create(new SmppBuffer(DefaultEncoding,
                            userDataProperty.DataBlock),
                        false);
                    userDataProperty.ValueString = userData.ShortMessageText(DefaultEncoding,
                        (DataCodings) dataCoding.ValueByte);
                ...
                {
                    PduData.ExtractTLV(details,
                        ref offset);

                    var tlvTag = details[details.Count - 3];
                    var tlvLength = details[details.Count - 2];
                    var tlvValue = details[details.Count - 1];

                    switch (tlvTag.ValueUShort)
                    {
                        case (ushort) OptionalTags.MessagePayload:
                            var messageFeature = (GSMSpecificFeatures) (esmClass.ValueByte & 0xc0);
                            var userData = new SmppBuffer(DefaultEncoding,
                                tlvValue.DataBlock);
                            userData.ExtractUserData(details,
                                messageFeature == GSMSpecificFeatures.UDHI,
                                offset);
                            break;
                        ...
                        case (ushort) OptionalTags.SarMsgRefNum:
                            ...
                            tlvValue.ValueUShort = SmppBuffer.BytesToShort(tlvValue.DataBlock,
                                0);
                            break;
        ...
        /// <summary>
        ///     Indicates if the message is a registered short message and thus if a Delivery Receipt is required upon the
        ...
        {
            get => UserData.ShortMessageText(DefaultEncoding,
                DataCoding);

            set => UserData.ShortMessage = new SmppBuffer(DefaultEncoding,
                value,
                DataCoding).Buffer;
        }

        /// <summary>
        ///     Up to 254 octets of short message user data. The exact physical limit for short_message size may vary
        ///     according to the underlying network
        /// </summary>
        public byte[] ShortMessageBytes
        {
            get => UserData.ShortMessage;
            set => UserData.ShortMessage = value;
        }

        /// <summary> Indicates message mode associated with the short message </summary>
        public MessageModes MessageMode
        {
            get => (MessageModes) (EsmClass & 0x03);
        ...
        {
            get => (MessageTypes) (EsmClass & 0x3c);
            set => EsmClass |= (byte) value;
        }
        ...
        public SMSCDeliveryReceipt SMSCReceipt
        {
            get => (SMSCDeliveryReceipt) (RegisteredDelivery & 0x03);
            set => RegisteredDelivery |= (byte) value;
        }

        /// <summary> SME originated Acknowledgement </summary>
        public SMEAcknowledgement Acknowledgement
        {
            get => (SMEAcknowledgement) (RegisteredDelivery & 0x0C);
            set => RegisteredDelivery |= (byte) value;
        }

        /// <summary> Intermediate Notification </summary>
        public IntermediateNotification Notification
        {
            get => (IntermediateNotification) (RegisteredDelivery & 0x10);
            set => RegisteredDelivery |= (byte) value;
        }
        ...
        private DeliverSm(DataCodings defaultEncoding) : base(defaultEncoding,
            CommandSet.DeliverSm,
        ...
        private DeliverSm(DataCodings defaultEncoding,
            ...
            byte destTon,
            byte destNpi,
            string destAddr) : base(defaultEncoding,
            CommandSet.DeliverSm,
            SequenceGenerator.Counter)
        {
            Optional = new TLVCollection();
        ...
        public static DeliverSm Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var deliverSm = new DeliverSm(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(deliverSm,
                    ref offset);
                ...
                if (deliverSm.ShortMessageLength > 0)
                    deliverSm.UserDataBuffer = new SmppBuffer(defaultEncoding,
                        buf.ExtractByteArray(ref offset,
                            deliverSm.ShortMessageLength));

                while (offset - start_offset < deliverSm.Length) deliverSm.Optional.Add(buf.ExtractTLV(ref offset));

                if (deliverSm.ShortMessageLength == 0)
                    if (deliverSm.Optional != null && deliverSm.Optional.Count > 0)
                    {
                        var tlv_payload = deliverSm.Optional[OptionalTags.MessagePayload];
                        if (tlv_payload != null)
                            deliverSm.UserDataBuffer = new SmppBuffer(defaultEncoding,
                                tlv_payload.Value);

                        var tlv_globalSeq = deliverSm.Optional[OptionalTags.SarMsgRefNum];
                        if (tlv_globalSeq != null)
                            deliverSm.MessageReferenceNumber = SmppBuffer.BytesToShort(tlv_globalSeq.Value,
                                0);

                        var tlv_pocketsNumber = deliverSm.Optional[OptionalTags.SarTotalSegments];
                        if (tlv_pocketsNumber != null) deliverSm.TotalSegments = tlv_pocketsNumber.Value[0];

                        var tlv_localSeq = deliverSm.Optional[OptionalTags.SarSegmentSeqnum];
                        if (tlv_localSeq != null) deliverSm.SeqmentNumber = tlv_localSeq.Value[0];
                    }

                if (deliverSm.UserDataBuffer != null && deliverSm.UserDataBuffer.Length > 0)
                {
                    var hasUDHI = deliverSm.MessageFeature == GSMSpecificFeatures.UDHI || deliverSm.MessageFeature == GSMSpecificFeatures.UDHIandReplyPath;
                    deliverSm.UserData = UserData.Create(deliverSm.UserDataBuffer,
                        hasUDHI);

                    if (deliverSm.UserData.Headers.Count > 0)
                    {
                        var udh = deliverSm.UserData.Headers[InformationElementIdentifiers.ConcatenatedShortMessages8bit];
                        if (udh != null)
                        {
                        ...
                        if (udh != null)
                        {
                            deliverSm.MessageReferenceNumber = SmppBuffer.BytesToShort(udh.Data,
                                0);
                ...
                else { deliverSm.UserData = UserData.Create(); }
            }

            catch { deliverSm = null; }
