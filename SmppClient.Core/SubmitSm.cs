...
namespace SmppClient.Core
{
    ...
    public class SubmitSm : Header, IPacket, IPduDetails
    {
        ...
        /// <summary>
        ///     Indicates how long the message is valid. Set to NULL to request the SMSC default validity period. Format
        ///     'YYMMDDhhmmsstnnp
        /// </summary>
        ...
        public List<PduPropertyDetail> Details()
        {
            ...
            try
            {
                ...
                details.Add(PduData.ExtractByte("DefaultMsgId",
                    ref offset));
            ...
            set
            {
                if (!string.IsNullOrEmpty(value))
                    if (!Regex.IsMatch(value,
                        "^[0-9][0-9][0-1][0-9][0-3][0-9][0-2][0-9][0-5][0-9][0-5][0-9][0-9][0-4][0-9][\\+\\-R]$")) throw new FormatException("Incorrect format. 'YYMMDDhhmmsstnnp'");
        ...
        private SubmitSm(DataCodings defaultEncoding) : base(defaultEncoding,
            CommandSet.SubmitSm,
        ...
        private SubmitSm(DataCodings defaultEncoding,
            string serviceType,
            byte sourceTon,
            byte sourceNpi,
            string sourceAddr,
        ...
        public static SubmitSm Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var submitSm = new SubmitSm(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(submitSm,
                    ref offset);
                ...
                if (submitSm.ShortMessageLength > 0)
                    submitSm.UserDataBuffer = new SmppBuffer(defaultEncoding,
                        buf.ExtractByteArray(ref offset,
                            submitSm.ShortMessageLength));

                while (offset - start_offset < submitSm.Length) submitSm.Optional.Add(buf.ExtractTLV(ref offset));

                if (submitSm.ShortMessageLength == 0)
                    if (submitSm.Optional != null && submitSm.Optional.Count > 0)
                    {
                        var tlv_payload = submitSm.Optional[OptionalTags.MessagePayload];
                        if (tlv_payload != null)
                            submitSm.UserDataBuffer = new SmppBuffer(defaultEncoding,
                                tlv_payload.Value);

                        var tlv_globalSeq = submitSm.Optional[OptionalTags.SarMsgRefNum];
                        if (tlv_globalSeq != null)
                            submitSm.MessageReferenceNumber = SmppBuffer.BytesToShort(tlv_globalSeq.Value,
                                0);

                        var tlv_pocketsNumber = submitSm.Optional[OptionalTags.SarTotalSegments];
                        if (tlv_pocketsNumber != null) submitSm.TotalSegments = tlv_pocketsNumber.Value[0];

                        var tlv_localSeq = submitSm.Optional[OptionalTags.SarSegmentSeqnum];
                        if (tlv_localSeq != null) submitSm.SeqmentNumber = tlv_localSeq.Value[0];
                    }

                if (submitSm.UserDataBuffer != null && submitSm.UserDataBuffer.Length > 0)
                {
                    var hasUDHI = submitSm.MessageFeature == GSMSpecificFeatures.UDHI || submitSm.MessageFeature == GSMSpecificFeatures.UDHIandReplyPath;
                    submitSm.UserData = UserData.Create(submitSm.UserDataBuffer,
                        hasUDHI);

                    if (submitSm.UserData.Headers.Count > 0)
                    {
                        var udh = submitSm.UserData.Headers[InformationElementIdentifiers.ConcatenatedShortMessages8bit];
                        if (udh != null)
                        {
                        ...
                        if (udh != null)
                        {
                            submitSm.MessageReferenceNumber = SmppBuffer.BytesToShort(udh.Data,
                                0);
                ...
                else { submitSm.UserData = UserData.Create(); }
            }
