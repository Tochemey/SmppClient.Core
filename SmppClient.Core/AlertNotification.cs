...
{
    /// <summary>
    ///     This message is sent by the SMSC (Short Message Service Centre) to the ESME (Extended Short Message Entity),
    ///     when the SMSC has detected that a particular mobile subscriber has become available and a delivery
    ///     pending flag had been set for that subscriber from a previous data_sm operation
    /// </summary>
    public class AlertNotification : Header, IPduDetails
    {
        ...
        public List<PduPropertyDetail> Details()
        {
            ...
            try
            {
                ...
                details.Add(PduData.ExtractByte("SourceAddrTon",
                    ref offset));
                details.Add(PduData.ExtractByte("SourceAddrNpi",
                    ref offset));
                details.Add(PduData.ExtractCString("SourceAddr",
                    ref offset));
                details.Add(PduData.ExtractByte("EsmeAddrTon",
                    ref offset));
                details.Add(PduData.ExtractByte("EsmeAddrNpi",
                    ref offset));
                details.Add(PduData.ExtractCString("EsmeAddr",
                    ref offset));
        ...
        public static AlertNotification Create(DataCodings defaultEncoding,
            SmppBuffer buf,
            ref int offset)
        {
            var alertNotification = new AlertNotification(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(alertNotification,
                    ref offset);
                ...
                while (offset - start_offset < alertNotification.Length) alertNotification.Optional.Add(buf.ExtractTLV(ref offset));
            }

            catch { alertNotification = null; }
