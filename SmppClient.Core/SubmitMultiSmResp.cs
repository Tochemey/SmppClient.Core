...
namespace SmppClient.Core
{
    ...
    public class SubmitMultiSmResp : Header, IPacket, IPduDetails
    {
        ...
        public static SubmitMultiSmResp Create(DataCodings defaultEncoding,
            SubmitMultiSm submitMultiSm,
            CommandStatus commandStatus,
            string messageId)
        {
            if (submitMultiSm.SourceAddr == string.Empty || submitMultiSm.SourceAddr == null) commandStatus = CommandStatus.ESME_RINVSRCADR;

            return new SubmitMultiSmResp(defaultEncoding,
                commandStatus,
                submitMultiSm.Sequence,
                messageId);
        }
        ...
        public static SubmitMultiSmResp Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var submitMultiResp = new SubmitMultiSmResp(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(submitMultiResp,
                    ref offset);

                if (submitMultiResp.Length > HEADER_LENGTH)
                {
                    ...
                    while (offset - start_offset < submitMultiResp.Length) submitMultiResp.Optional.Add(buf.ExtractTLV(ref offset));
                }
            }

            catch { submitMultiResp = null; }
