...
{
    /// <summary>
    ///     This command is used to transfer data between the SMSC (Short Message Service Centre) and the
    ///     ESME (Extended Short Message Entity). It may be used by both the ESME and SMSC
    /// </summary>
    public class DataSm : Header, IPacket, IPduDetails
    {
        ...
        public List<PduPropertyDetail> Details()
        {
            ...
            try
            {
                ...
                details.Add(PduData.ExtractByte("EsmClass",
                    ref offset));
                details.Add(PduData.ExtractByte("RegisteredDelivery",
                    ref offset));
                details.Add(PduData.ExtractByte("DataCoding",
                    ref offset));
        ...
        public static DataSm Create(DataCodings defaultEncoding,
            SmppBuffer buf,
            ref int offset)
        {
            var dataSm = new DataSm(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(dataSm,
                    ref offset);
                ...
                {
                    if (dataSm.Optional == null) dataSm.Optional = new TLVCollection();

                    while (offset - start_offset < dataSm.Length) dataSm.Optional.Add(buf.ExtractTLV(ref offset));
                }

                if (dataSm.Optional != null && dataSm.Optional.Count > 0)
                {
                    var tlv_payload = dataSm.Optional[OptionalTags.MessagePayload];
                    if (tlv_payload != null)
                        dataSm.UserDataBuffer = new SmppBuffer(defaultEncoding,
                            tlv_payload.Value);

                    var tlv_globalSeq = dataSm.Optional[OptionalTags.SarMsgRefNum];
                    if (tlv_globalSeq != null)
                        dataSm.MessageReferenceNumber = SmppBuffer.BytesToShort(tlv_globalSeq.Value,
                            0);

                    var tlv_pocketsNumber = dataSm.Optional[OptionalTags.SarTotalSegments];
                    if (tlv_pocketsNumber != null) dataSm.TotalSegments = tlv_pocketsNumber.Value[0];

                    var tlv_localSeq = dataSm.Optional[OptionalTags.SarSegmentSeqnum];
                    if (tlv_localSeq != null) dataSm.SequenceNumber = tlv_localSeq.Value[0];
                }

                if (dataSm.UserDataBuffer != null && dataSm.UserDataBuffer.Length > 0)
                    dataSm.UserData = UserData.Create(dataSm.UserDataBuffer,
                        dataSm.MessageFeature == GSMSpecificFeatures.UDHI);
                else dataSm.UserData = UserData.Create();
            }
