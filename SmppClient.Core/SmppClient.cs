...
namespace SmppClient.Core
{
    ...
    public class SmppClient : IDisposable
    {
        ...
        public DeliverSm PrepareDeliver(DeliverMode mode,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string message)
        {
            ...
            {
                OnError(string.Format("Unsupported data coding [{0}]",
                    encodeDataCoding));
                encodeDataCoding = DataCodings.Default;
            }

            // Standard 160 bytes
            var maxBytes = ShortMessageMaxBytes;
            if (encodeDataCoding == DataCodings.UCS2) maxBytes = 140;

            // Convert the message to a byte array
            var messageBytes = new SmppBuffer(DefaultEncoding,
                message,
                encodeDataCoding).Buffer;

            switch (mode)
            {
                case DeliverMode.ShortMessage:
                    deliverSm = DeliverSm.Create(DefaultEncoding,
                        serviceType,
                        srcTon,
                        srcNpi,
                        srcAddr,
                    ...
                    {
                        var memory = new List<byte>(messageBytes);
                        deliverSm.ShortMessageBytes = memory.GetRange(0,
                            160).ToArray();
                    }
                    else { deliverSm.ShortMessageBytes = messageBytes; }
                    break;
        ...
        public void SendPacket(IPacket packet)
        {
            try
            {
                OnPduDetails(PduDirectionTypes.Sent,
                    (Header) packet);
        ...
        public void ProcessReceivedData()
        {
            try
            {
                ...
                var sb = new SmppBuffer(DefaultEncoding,
                    _SocketClient.MessageBuffer.ToArray());
                var offset = 0;
                ...
                while (offset < sb.Length)
                {
                    ...
                    if (sb.Length - offset < Header.HEADER_LENGTH)
                    {
                        ...
                        _SocketClient.MessageBuffer.Write(sb.Buffer,
                            offset,
                            sb.Length - offset);
                        _SocketClient.MessageBuffer.Seek(0,
                            SeekOrigin.End);
                    ...
                    var commandLength = Convert.ToInt32(hd.Length);
                    ...
                    switch (hd.Command)
                    {
                        case CommandSet.AlertNotification:
                            ...
                            var alertn = AlertNotification.Create(DefaultEncoding,
                                sb,
                                ref offset);

                            OnPduDetails(PduDirectionTypes.Received,
                                alertn);
                        ...
                        case CommandSet.BindTransmitterResp:
                            ...
                            BindSmResp = BindSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (BindSmResp == null)
                            {
                        ...
                        case CommandSet.BindTransmitter:
                            ...
                            var bindSm = BindSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (bindSm != null)
                            {
                                OnPduDetails(PduDirectionTypes.Received,
                                    bindSm);
                                var commandStatus = OnBindSm(bindSm);

                                var commandSet = bindSm.Command == CommandSet.BindTransmitter ? CommandSet.BindTransmitterResp : bindSm.Command == CommandSet.BindReceiver ? CommandSet.BindReceiverResp : CommandSet.BindTransceiverResp;

                                SendPacket(BindSmResp.Create(DefaultEncoding,
                                    bindSm,
                                    commandSet,
                                    commandStatus,
                                    "AS"));
                        ...
                        case CommandSet.CancelSm:
                            ...
                            var cancelSm = CancelSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (cancelSm != null)
                            {
                                OnPduDetails(PduDirectionTypes.Received,
                                    cancelSm);
                                var commandStatus = OnCancelSm(cancelSm);

                                SendPacket(CancelSmResp.Create(DefaultEncoding,
                                    cancelSm,
                                    commandStatus));
                            }
                            else
                        ...
                        case CommandSet.CancelSmResp:
                            ...
                            var cancelResp = CancelSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (cancelResp == null)
                                cancelResp = CancelSmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP);
                        ...
                        case CommandSet.DataSm:
                            ...
                            var dataSm = DataSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (dataSm != null)
                            {
                                ...
                                var commandStatus = OnDataSm(dataSm,
                                    out messageId);

                                SendPacket(DataSmResp.Create(DefaultEncoding,
                                    dataSm,
                                    commandStatus,
                                    messageId));
                            }
                        ...
                        case CommandSet.DataSmResp:
                            ...
                            var dataSmResp = DataSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (dataSmResp == null)
                                dataSmResp = DataSmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP);
                        ...
                        case CommandSet.DeliverSm:
                            ...
                            var deliverSm = DeliverSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (deliverSm != null)
                            {
                                OnPduDetails(PduDirectionTypes.Received,
                                    deliverSm);
                                var commandStatus = OnDeliverSm(deliverSm);

                                SendPacket(DeliverSmResp.Create(DefaultEncoding,
                                    deliverSm,
                                    commandStatus));
                            }
                            else
                        ...
                        case CommandSet.DeliverSmResp:
                            ...
                            var deliverSmResp = DeliverSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (deliverSmResp == null)
                                deliverSmResp = DeliverSmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP);
                        ...
                        case CommandSet.EnquireLink:
                            ...
                            var enquireLink = EnquireLinkSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (enquireLink != null)
                            {
                                OnPduDetails(PduDirectionTypes.Received,
                                    enquireLink);
                                var commandStatus = OnEnquireLinkSm(enquireLink);

                                SendPacket(EnquireLinkSmResp.Create(DefaultEncoding,
                                    enquireLink,
                                    commandStatus));
                            }
                            else
                        ...
                        case CommandSet.EnquireLinkResp:
                            ...
                            EnquireLinkSmResp = EnquireLinkSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                        ...
                        case CommandSet.GenericNack:
                            ...
                            var nack = GenericNackSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (nack == null)
                                nack = GenericNackSm.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP,
                                    hd.Sequence);
                        ...
                        case CommandSet.QuerySm:
                            ...
                            var querySm = QuerySm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (querySm != null)
                            {
                            ...
                            {
                                SendPacket(QuerySmResp.Create(DefaultEncoding,
                                    CommandStatus.ESME_RSYSERR,
                                    hd.Sequence));
                        ...
                        case CommandSet.QuerySmResp:
                            ...
                            var querySmResp = QuerySmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (querySmResp == null)
                                querySmResp = QuerySmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP);
                        ...
                        case CommandSet.SubmitSm:
                            ...
                            var submitSm = SubmitSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (submitSm != null)
                            {
                                ...
                                string messageId = null;
                                var commandStatus = OnSubmitSm(submitSm,
                                    out messageId);

                                SendPacket(SubmitSmResp.Create(DefaultEncoding,
                                    submitSm,
                                    commandStatus,
                                    messageId));
                            }
                        ...
                        case CommandSet.SubmitSmResp:
                            ...
                            var submitSmResp = SubmitSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (submitSmResp == null)
                                submitSmResp = SubmitSmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP,
                            ...
                            lock (SubmitDone)
                            {
                                if (SubmitDone.ContainsKey(submitSmResp.Sequence))
                                {
                                    var eventData = SubmitDone[submitSmResp.Sequence];
                        ...
                        case CommandSet.SubmitMultiSm:
                            ...
                            var submitMultiSm = SubmitMultiSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (submitMultiSm != null)
                            {
                                ...
                                string messageId = null;
                                var commandStatus = OnSubmitMultiSm(submitMultiSm,
                                    out messageId);

                                SendPacket(SubmitMultiSmResp.Create(DefaultEncoding,
                                    submitMultiSm,
                                    commandStatus,
                                    messageId));
                            }
                        ...
                        case CommandSet.SubmitMultiSmResp:
                            ...
                            var submitMultiSmResp = SubmitMultiSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (submitMultiSmResp == null)
                                submitMultiSmResp = SubmitMultiSmResp.Create(DefaultEncoding,
                                    CommandStatus.SMPPCLIENT_UNEXPRESP,
                                    submitMultiSmResp.Sequence);
                            ...
                            lock (SubmitDone)
                            {
                                if (SubmitDone.ContainsKey(submitMultiSmResp.Sequence))
                                {
                                    var eventData = SubmitDone[submitMultiSmResp.Sequence];
                        ...
                        case CommandSet.Unbind:
                            ...
                            var unBindSm = UnBindSm.Create(DefaultEncoding,
                                sb,
                                ref offset);
                            if (unBindSm != null)
                            {
                                ...
                                SendPacket(UnBindSmResp.Create(DefaultEncoding,
                                    unBindSm,
                        ...
                        case CommandSet.UnbindResp:
                            ...
                            UnBindSmResp = UnBindSmResp.Create(DefaultEncoding,
                                sb,
                                ref offset);
                        ...
                        default:
                            offset++;
                            OnError("Undefined packet received : " + hd.Command + " with status " + hd.Status);
                            break;
                    }
                ...
                lock (_LastPduLock) { _LastPdu = DateTime.Now; }
            }
        ...
        public delegate void AlertNotificationEventHandler(object sender,
        ...
        public delegate CommandStatus BindSmEventHandler(object sender,
            BindSm data);

        /// <summary> Occurs when Bind Response packet received from SMSC </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void BindSmRespEventHandler(object sender,
            BindSmResp data);
        ...
        /// <param name="data"></param>
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus CancelSmEventHandler(object sender,
            CancelSm data);

        /// <summary> Occurs when received CancelSmResp PDU </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void CancelSmRespEventHandler(object sender,
            CancelSmResp data);
        ...
        /// <param name="data"></param>
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus DeliverSmEventHandler(object sender,
            DeliverSm data);

        /// <summary> Occurs when received DeliverSmResp PDU </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void DeliverSmRespEventHandler(object sender,
            DeliverSmResp data);
        ...
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus DataSmEventHandler(object sender,
            DataSm data,
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void DataSmRespEventHandler(object sender,
            DataSmResp data);
        ...
        /// <param name="data"></param>
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus EnquireLinkSmEventHandler(object sender,
            EnquireLinkSm data);

        /// <summary> Occurs when received EnquireLinkSmResp PDU </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void EnquireLinkSmRespEventHandler(object sender,
            EnquireLinkSmResp data);
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void GenericNackSmEventHandler(object sender,
            GenericNackSm data);
        ...
        public delegate Guid? PduDetailsEventHandler(object sender,
            PduDirectionTypes pduDirectionType,
            Header pdu,
        ...
        public delegate void QuerySmEventHandler(object sender,
            QuerySm data,
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void QuerySmRespEventHandler(object sender,
            QuerySmResp data);

        /// <summary> Occurs when any PDU was sent from ESME to SMSC </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void SendDataEventHandler(object sender,
            byte[] data);
        ...
        /// <param name="data"></param>
        /// <param name="messageId"></param>
        public delegate CommandStatus SubmitSmEventHandler(object sender,
            SubmitSm data,
            out string messageId);
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void SubmitSmRespEventHandler(object sender,
            SubmitSmResp data);
        ...
        /// <param name="messageId"></param>
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus SubmitMultiSmEventHandler(object sender,
            SubmitMultiSm data,
            out string messageId);
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void SubmitMultiSmRespEventHandler(object sender,
            SubmitMultiSmResp data);

        /// <summary> Occurs when any PDU was received from SMSC to ESME </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void ReceiveDataEventHandler(object sender,
            byte[] data);
        ...
        /// <param name="data"></param>
        /// <returns> CommandStatus </returns>
        public delegate CommandStatus UnBindSmEventHandler(object sender,
            UnBindSm data);

        /// <summary> Occurs when Unbind Response packet received from SMSC </summary>
        /// <param name="sender"></param>
        /// <param name="data"></param>
        public delegate void UnBindSmRespEventHandler(object sender,
            UnBindSmResp data);
        ...
        /// <summary>
        ///     Defines maximum bytes length in one part of the concatenated short messages. The GSM 7bit standard defines a
        ///     maximum of 160 octets for a single short message
        /// </summary>
        ...
        private readonly object _LastPduLock = new object();
        ...
        private BindSmResp BindSmResp;

        /// <summary> Response to the unbind request </summary>
        private UnBindSmResp UnBindSmResp;

        /// <summary> Response to the enquire link request </summary>
        private EnquireLinkSmResp EnquireLinkSmResp;
        ...
        private readonly ManualResetEvent GenericDone = new ManualResetEvent(false);

        /// <summary> Event to signal connection is done </summary>
        private ManualResetEvent ConnectDone = new ManualResetEvent(false);

        /// <summary> Event to signal bind is done </summary>
        private readonly ManualResetEvent BindDone = new ManualResetEvent(false);

        /// <summary> Event to signal unbind is done </summary>
        private readonly ManualResetEvent UnbindDone = new ManualResetEvent(false);

        /// <summary> Event to signal enquire is done </summary>
        private readonly ManualResetEvent EnquireDone = new ManualResetEvent(false);

        /// <summary> Events to signal submit is done </summary>
        private readonly Dictionary<uint, EventData> SubmitDone = new Dictionary<uint, EventData>();

        /// <summary> A set of events for processing </summary>
        private readonly Stack<EventData> EventDataList = new Stack<EventData>();
        ...
        /// <summary>
        ///     Defines maximum bytes length in one part of the concatenated short messages.  The GSM standard defines a
        ///     maximum of 140 octets for a single short message
        /// </summary>
        public int ShortMessageMaxBytes
        {
            get => _ShortMessageMaxBytes;
        ...
        public SmppClient(DataCodings defaultEncoding)
        {
            ...
            _SocketClient = new SocketClient(10485760,
                0,
                this,
                MessageHandler,
                CloseHandler,
            ...
            _EnquireLinkTimer = new SynchronousTimer(SendEnquireLink,
                null,
                _EnqInterval);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_Disposed)
            {
                if (disposing)
                {
                    ...
                    lock (SubmitDone) { foreach (var eventData in SubmitDone.Values) eventData.SignalEvent.Set(); }
                }
        ...
        private EventData RequestEventData()
        {
            ...
            lock (EventDataList)
            {
                if (EventDataList.Count == 0) eventData = new EventData();
                else eventData = EventDataList.Pop();
            }
        ...
        private void MessageHandler(SocketClient socket)
        {
            try
            {
                ...
                socket.MessageBuffer.Write(_SocketClient.RawBuffer,
                    0,
                    _SocketClient.MessageLength);
        ...
        private void ErrorHandler(SocketClient socket,
            Exception exception)
        {
            try
            {
        ...
        {
            var commandStatus = CommandStatus.ESME_RBINDFAIL;

            try
            {
                if (BindSmEvent != null)
                {
                    commandStatus = BindSmEvent(this,
                        data);

                    if (commandStatus == CommandStatus.ESME_ROK) _Status = ConnectionStatus.Bound;
                }
            }
        ...
        {
            var commandStatus = CommandStatus.ESME_RCANCELFAIL;

            try
            {
                if (CancelSmEvent != null)
                    commandStatus = CancelSmEvent(this,
                        data);
            }
        ...
        /// <param name="messageId"></param>
        /// <returns> CommandStatus </returns>
        internal CommandStatus OnDataSm(DataSm data,
            out string messageId)
        {
            ...
            {
                if (DataSmEvent != null)
                    commandStatus = DataSmEvent(this,
                        data,
                        out messageId);
            }
        ...
        internal CommandStatus OnDeliverSm(DeliverSm data)
        {
            ...
            try
            {
                if (DeliverSmEvent != null)
                    commandStatus = DeliverSmEvent(this,
                        data);
            }
        ...
        {
            try { if (DisconnectEvent != null) DisconnectEvent(this); }
        ...
        internal CommandStatus OnEnquireLinkSm(EnquireLinkSm data)
        {
            ...
            try
            {
                if (EnquireLinkSmEvent != null)
                    commandStatus = EnquireLinkSmEvent(this,
                        data);
            }
        ...
        /// <param name="pduDirectionType"></param>
        /// <param name="pdu"></param>
        internal void OnPduDetails(PduDirectionTypes pduDirectionType,
            Header pdu)
        {
            try
            {
                if (PduDetailsEvent != null)
                {
                    if (pduDirectionType == PduDirectionTypes.Sent)
                        pdu.PduData = new SmppBuffer(DefaultEncoding,
                            ((IPacket) pdu).GetPDU());

                    pdu.ExternalId = PduDetailsEvent(this,
                        pduDirectionType,
                        pdu,
                        ((IPduDetails) pdu).Details());
                }
            }
        ...
        /// <param name="messageId"></param>
        /// <returns> CommandStatus </returns>
        internal CommandStatus OnSubmitSm(SubmitSm data,
            out string messageId)
        {
            var commandStatus = CommandStatus.ESME_RSUBMITFAIL;
            messageId = null;

            try
            {
                if (SubmitSmEvent != null)
                    commandStatus = SubmitSmEvent(this,
                        data,
                        out messageId);
            }
        ...
        /// <param name="messageId"></param>
        /// <returns> CommandStatus </returns>
        internal CommandStatus OnSubmitMultiSm(SubmitMultiSm data,
            out string messageId)
        {
            var commandStatus = CommandStatus.ESME_RSUBMITFAIL;
            messageId = null;
            ...
            try
            {
                if (SubmitMultiSmEvent != null)
                    commandStatus = SubmitMultiSmEvent(this,
                        data,
                        out messageId);
            }
        ...
        {
            var commandStatus = CommandStatus.ESME_RSYSERR;

            try
            {
                if (UnBindSmEvent != null)
                    commandStatus = UnBindSmEvent(this,
                        data);
            }
            ...
            int port)
        {
            ...
            try
            {
                ...
                _SocketClient.Connect(hostName,
                    port);
        ...
        public BindSmResp Bind(string SystemId,
            string Password)
        {
            return Bind(SystemId,
                Password,
                ConnectionModes.Transceiver);
        }
        ...
        {
            if (SystemId == null || SystemId.Length == 0) throw new ArgumentException("SystemId");
            ...
            var bindSmResp = BindSmResp.Create(DefaultEncoding);
        ...
        public UnBindSmResp UnBind()
        {
            ...
            try
            {
                if (_Status != ConnectionStatus.Bound)
                    return UnBindSmResp.Create(DefaultEncoding,
                        CommandStatus.SMPPCLIENT_NOCONN);
                ...
                if (res == 0)
                {
                    ...
                    if (unBindSmResp.Status == CommandStatus.ESME_ROK) _Status = ConnectionStatus.Open;
                }
                else if (res == 1)
                {
                    unBindSmResp = UnBindSmResp.Create(DefaultEncoding,
                        CommandStatus.SMPPCLIENT_GENERIC_NACK);
                }
                else
                {
                    unBindSmResp = UnBindSmResp.Create(DefaultEncoding,
                        CommandStatus.SMPPCLIENT_RCVTIMEOUT);
                }
            }

            catch (Exception exception)
            {
                OnError("UnBind error : ",
                    exception);

                unBindSmResp = UnBindSmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_UNKNOWNERROR);
            }
        ...
        public SubmitSmResp Submit(SubmitSm data)
        {
            ...
            if (_Status != ConnectionStatus.Bound)
                return SubmitSmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_UNBOUND,
                    data.Sequence);
            ...
            try
            {
                ...
                // Wait the response
                var success = eventData.SignalEvent.WaitOne(5000);

                if (success) submitSmResp = (SubmitSmResp) eventData.ResponseObject;
                else
                    submitSmResp = SubmitSmResp.Create(DefaultEncoding,
                        CommandStatus.SMPPCLIENT_RCVTIMEOUT,
                        data.Sequence);
            ...
            lock (SubmitDone) { SubmitDone.Remove(data.Sequence); }
        ...
        public SubmitMultiSmResp Submit(SubmitMultiSm data)
        {
            ...
            if (_Status != ConnectionStatus.Bound)
                return SubmitMultiSmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_UNBOUND,
                    data.Sequence);
            ...
            try
            {
                ...
                if (success) submitMultiSmResp = (SubmitMultiSmResp) eventData.ResponseObject;
                else
                    submitMultiSmResp = SubmitMultiSmResp.Create(DefaultEncoding,
                        CommandStatus.SMPPCLIENT_RCVTIMEOUT,
                        data.Sequence);
        ...
        {
            var list = new List<SubmitSmResp>();

            foreach (var submitSm in submitSmList)
            {
                var resp = Submit(submitSm);

                list.Add(resp);

                if (resp.Status != CommandStatus.ESME_ROK) break;
            }
        ...
        {
            var list = new List<SubmitMultiSmResp>();

            foreach (var submitMulti in submitMultiList)
            {
                var resp = Submit(submitMulti);
        ...
        public SubmitSm PrepareSubmit(SubmitMode mode,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string message)
        {
            ...
            switch (mode)
            {
                case SubmitMode.ShortMessage:
                    submitSm = SubmitSm.Create(DefaultEncoding,
                        serviceType,
                        srcTon,
                        srcNpi,
                        srcAddr,
                    ...
                    {
                        var memory = new List<byte>(messageBytes);
                        submitSm.ShortMessageBytes = memory.GetRange(0,
                            160).ToArray();
                    }
                    else { submitSm.ShortMessageBytes = messageBytes; }
                    break;
        ...
        public List<SubmitSm> PrepareSubmitLarge(SubmitMode mode,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string message)
        {
            ...
            switch (mode)
            {
                case SubmitMode.ShortMessage:
                    ...
                    else
                    {
                        ...
                        foreach (var part in parts)
                        {
                            ...
                            submitSm.DataCoding = submitDataCoding;
                            submitSm.UserData.Headers.AddConcatenatedShortMessages8bit(DefaultEncoding,
                                messageReference,
                                Convert.ToByte(parts.Count),
                                Convert.ToByte(sequenceNumber));
                            submitSm.ShortMessageBytes = new SmppBuffer(DefaultEncoding,
                                part,
                                encodeDataCoding).Buffer;
                            submitSmList.Add(submitSm);
                ...
                case SubmitMode.Payload:
                    ...
                    else
                    {
                        ...
                        if (encodeDataCoding == DataCodings.Default || encodeDataCoding == DataCodings.ASCII || encodeDataCoding == DataCodings.Latin1 || encodeDataCoding == DataCodings.Latin1Escape || encodeDataCoding == DataCodings.DefaultFlashSMS) maxBytes = Convert.ToInt32(Math.Floor(Convert.ToDouble(maxBytes) * 8 / 7));

                        var messageReference = SequenceGenerator.ByteCounter;
                        var sequenceNumber = 1;

                        // Split the message in parts we can send
                        var parts = SmppBuffer.SplitMessageOnParts(messageBytes,
                            maxBytes);
            ...
            foreach (var submitSm in submitSmList) submitSm.ValidityPeriod = DateTime.Now.AddDays(2).ToString("yyMMddhhmmss000+");
        ...
        public SubmitMultiSm PrepareSubmit(SubmitMode mode,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string message)
        {
            ...
            {
                case SubmitMode.ShortMessage:
                    submitMulti = SubmitMultiSm.Create(DefaultEncoding,
                        serviceType,
                        srcTon,
                        srcNpi,
                        srcAddr,
                    ...
                    {
                        var memory = new List<byte>(messageBytes);
                        submitMulti.ShortMessageBytes = memory.GetRange(0,
                            160).ToArray();
                    }
                    else { submitMulti.ShortMessageBytes = messageBytes; }
                    break;
        ...
        /// <returns> A SubmitSm object </returns>
        public SubmitSm PrepareWapPush(byte transactionId,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string title)
        {
            ...
            else { sb.AddByte(0x0b); }

            var parts = Regex.Split(urlTmp,
                @"(\.com/|\.org/\.edu/|\.net/)");
            foreach (var part in parts)
                if (part == ".com/") sb.AddByte(0x85);
                else if (part == ".edu/") sb.AddByte(0x86);
                else if (part == ".net/") sb.AddByte(0x87);
                else if (part == ".org/") sb.AddByte(0x88);
                else
                    sb.AddEncodedString(part,
                        DataCodings.Octets);

            sb.AddByte(0x07); // action='signal-medium'
            sb.AddByte(0x01); // END attribute list
            sb.AddEncodedString(title,
                DataCodings.Octets);
            ...
            {
                sp = SubmitSm.Create(DefaultEncoding,
                    serviceType,
                    srcTon,
                    srcNpi,
                    srcAddr,
                ...
                sp.UserData.Headers.Add(InformationElementIdentifiers.ApplicationPortAddressingScheme16bit,
                    new byte[]
                    {
                        0x0b,
                        0x84,
            ...
            {
                throw new ArgumentException("Message too large to send",
                    "message_length");
            }
        ...
        public CancelSmResp Cancel(string serviceType,
            string messageId,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
            ...
            string dstAddr)
        {
            return Cancel(CancelSm.Create(DefaultEncoding,
                serviceType,
                messageId,
                srcTon,
                srcNpi,
        ...
        {
            var resp = CancelSmResp.Create(DefaultEncoding,
                CommandStatus.ESME_ROK);

            if (_Status != ConnectionStatus.Bound)
            {
                resp = CancelSmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_UNBOUND);
                return resp;
            }
        ...
        /// <returns> QuerySmResp </returns>
        public QuerySmResp Query(string messageId,
            byte sourceTon,
            byte sourceNpi,
            string sourceAddr)
        {
            return Query(QuerySm.Create(DefaultEncoding,
                messageId,
                sourceTon,
                sourceNpi,
                sourceAddr));
        ...
        {
            var resp = QuerySmResp.Create(DefaultEncoding,
                data,
                CommandStatus.ESME_ROK);

            if (_Status != ConnectionStatus.Bound)
            {
                resp = QuerySmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_UNBOUND);
                return resp;
            }
        ...
        private void SendEnquireLink(object stateInfo,
        ...
        public EnquireLinkSmResp PerformEnquireLink()
        {
            ...
            var data = EnquireLinkSm.Create(DefaultEncoding);

            OnEnquireLinkSm(data);

            SendPacket(data);

            if (EnquireDone.WaitOne(Timeout,
                false)) resp = EnquireLinkSmResp;
            else
                resp = EnquireLinkSmResp.Create(DefaultEncoding,
                    CommandStatus.SMPPCLIENT_RCVTIMEOUT,
                    0);
