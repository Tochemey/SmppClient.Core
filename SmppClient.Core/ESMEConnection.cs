...
namespace SmppClient.Core
{
    ...
    internal class EsmeConnection : IDisposable
    {
        ...
        public delegate void RECEIVED_MESSAGE_HANDLER(string logKey,
            string serviceType,
            Ton sourceTon,
            Npi sourceNpi,
        ...
        public delegate void RECEIVED_GENERICNACK_HANDLER(string logKey,
            int sequence);
        ...
        public delegate void SUBMIT_MESSAGE_HANDLER(string logKey,
            int sequence,
            string messageId);
        ...
        public delegate void QUERY_MESSAGE_HANDLER(string logKey,
            int sequence,
            string messageId,
            DateTime finalDate,
            int messageState,
        ...
        /// <param name="message"></param>
        /// <param name="logKey"></param>
        public delegate void LOG_EVENT_HANDLER(LogEventNotificationTypes logEventNotificationType,
            string logKey,
            string shortLongCode,
            string message);
        ...
        /// <param name="message"></param>
        public delegate void CONNECTION_EVENT_HANDLER(string logKey,
        ...
        public delegate Guid? PDU_DETAILS_EVENT_HANDLER(string logKey,
        ...
        private readonly string ShortLongCode;

        /// <summary> The connection mode, Transceiver, Transmitter, Receiver</summary>
        private readonly ConnectionModes ConnectionMode;
        ...
        private readonly RECEIVED_MESSAGE_HANDLER ReceivedMessageHandler;

        /// <summary> A user supplied method to call when a generic nack is received </summary>
        private readonly RECEIVED_GENERICNACK_HANDLER ReceivedGenericNackHandler;

        /// <summary> A user supplied method to call when a submit is ascknowledged </summary>
        private readonly SUBMIT_MESSAGE_HANDLER SubmitMessageHandler;

        /// <summary> A user supplied method to call when a query is responded </summary>
        private readonly QUERY_MESSAGE_HANDLER QueryMessageHandler;

        /// <summary> A user supplied method to call to write logs </summary>
        private readonly LOG_EVENT_HANDLER LogEventHandler;

        /// <summary> A user supplied method to call for connection events </summary>
        private readonly CONNECTION_EVENT_HANDLER ConnectionEventHandler;

        /// <summary> A user supplied method to call for pdu detail data </summary>
        private readonly PDU_DETAILS_EVENT_HANDLER PduDetailsEventHandler;
        ...
        private readonly ManualResetEvent ConnectEvent = new ManualResetEvent(true);

        /// <summary> Dictionary of packet data from cell phones </summary>
        private readonly Dictionary<string, SmppClient.UserDataControl> Collector = new Dictionary<string, SmppClient.UserDataControl>();
        ...
        public int ConnectionIdentifier => ConnectionId;

        /// <summary> The current status of the connection </summary>
        public ConnectionStatus Status => Client.Status;
        ...
        public EsmeConnection(int connectionId,
            string shortLongCode,
            ...
            PDU_DETAILS_EVENT_HANDLER pduDetailsEventHandler)
        {
            ...
            Client.ConnectEvent += ClientEventConnect;
            Client.DeliverSmEvent += ClientEventDeliverSm;
            Client.DisconnectEvent += ClientEventDisconnect;
            Client.EnquireLinkSmEvent += ClientEventEnquireLinkSm;
            Client.EnquireLinkSmRespEvent += ClientEventEnquireLinkSmResp;
            Client.ErrorEvent += ClientEventError;
            Client.GenericNackSmEvent += ClientEventGenericNackSm;
            Client.QuerySmRespEvent += ClientEventQuerySmResp;
            Client.SubmitSmRespEvent += ClientEventSubmitSmResp;
            Client.UnBindSmEvent += ClientEventUnBindSm;
            Client.PduDetailsEvent += ClientEventPduDetails;

            // Start a thread to get this connection
            ConnectionThread = new Thread(PerformConnectClient);
            ...
            if (!Disposed)
            {
                ...
                catch (Exception exception)
                {
                    WriteLog(LogEventNotificationTypes.Email,
                        "ESMEConnection : Dispose : ERROR : {0}",
                        exception.ToString());
                }
        ...
        private void WriteLog(string message)
        {
            if (LogEventHandler != null)
                LogEventHandler(LogEventNotificationTypes.None,
                    LogKey,
                    ShortLongCode,
                    message);
        }
        ...
        /// <param name="logEventNotificationType"></param>
        /// <param name="message"></param>
        private void WriteLog(LogEventNotificationTypes logEventNotificationType,
            string message)
        {
            if (LogEventHandler != null)
                LogEventHandler(logEventNotificationType,
                    LogKey,
                    ShortLongCode,
                    message);
        }
        ...
        private void PerformConnectClient()
        {
            for (;;)
                ...
                {
                    WriteLog("ESMEConnection : PerformConnectClient : Info : Dropping the Connection : {0}",
                        exception.Message);
        ...
        {
            WriteLog("ESMEConnection : Connect : Started : Host[{0}] Port[{1}]",
                Host,
                Port);
            ...
            try
            {
                if (Client.Status == ConnectionStatus.Closed)
                {
                    ...
                    retVal = Client.Connect(Host,
                        Port);

                    if (Client.Status != ConnectionStatus.Closed)
                    {
                        ConnectionEventHandler(LogKey,
                            ConnectionEventTypes.Connected,
                            string.Format("ESMEConnection : Connect : Info : Host[{0}] Port[{1}] Connection Established",
                                Host,
                                Port));
                        retVal = true;
                    }
                    ...
                    {
                        ConnectionEventHandler(LogKey,
                            ConnectionEventTypes.ConnectionAttemptFailed,
                            string.Format("ESMEConnection : Connect : WARNING : Host[{0}] Port[{1}] Connection Failure",
                                Host,
                                Port));
                    }
                }
                ...
                {
                    WriteLog("ESMEConnection : Connect : Info : Host[{0}] Port[{1}] Connection Already Established",
                        Host,
                        Port);
                    retVal = true;
                }
            ...
            {
                WriteLog("ESMEConnection : Connect : ERROR : Host[{0}] Port[{1}] Connection Failed {2}",
                    Host,
                    Port,
                    exception.Message);
            }

            WriteLog("ESMEConnection : Connect : Completed : Host[{0}] Port[{1}] RetVal[{2}]",
                Host,
                Port,
                retVal);
        ...
        {
            WriteLog("ESMEConnection : Disconnect : Started : Host[{0}] Port[{1}]",
                Host,
                Port);

            try
            {
                if (Client.Status == ConnectionStatus.Bound) if (!UnBind()) Client.Disconnect();

                if (Client.Status == ConnectionStatus.Open) Client.Disconnect();
            }

            catch (Exception exception)
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : Disconnect : ERROR : {0}",
                    exception.Message);
            }

            WriteLog("ESMEConnection : Disconnect : Completed : Host[{0}] Port[{1}]",
                Host,
                Port);
        }
        ...
        private bool Bind()
        {
            WriteLog("ESMEConnection : Bind : Started : Host[{0}] Port[{1}]",
                Host,
                Port);
            ...
            try
            {
                ...
                switch (btrp.Status)
                {
                    case CommandStatus.ESME_ROK:
                        IsBound = true;
                        ConnectionEventHandler(LogKey,
                            ConnectionEventTypes.Bound,
                            string.Format("ESMEConnection : Bind : Info : Host[{0}] Port[{1}] Bind Established",
                                Host,
                                Port));
                    ...
                    default:
                        ConnectionEventHandler(LogKey,
                            ConnectionEventTypes.BindingAttemptFailed,
                            string.Format("ESMEConnection : Bind : ERROR : Host[{0}] Port[{1}] Status[{2}]",
                                Host,
                                Port,
                                btrp.Status.ToString()));
                        break;
                }
            }
            ...
            {
                WriteLog("ESMEConnection : Bind : ERROR : Host[{0}] Port[{1}] Bind Failed {2}",
                    Host,
                    Port,
                    exception.Message);
            }

            WriteLog("ESMEConnection : Bind : Completed : Host[{0}] Port[{1}] RetVal[{2}]",
                Host,
                Port,
                retVal);
        ...
        private bool UnBind()
        {
            WriteLog("ESMEConnection : UnBind : Started : Host[{0}] Port[{1}]",
                Host,
                Port);
            ...
            try
            {
                var ubtrp = Client.UnBind();

                switch (ubtrp.Status)
                {
                    case CommandStatus.ESME_ROK:
                        WriteLog("ESMEConnection : UnBind : Info : Host[{0}] Port[{1}] Connection UnBound",
                            Host,
                            Port);
                        retValue = true;
                        break;
                    ...
                    default:
                        WriteLog("ESMEConnection : UnBind : WARNING : Host[{0}] Port[{1}] Status[{2}] Connection UnBound Failure",
                            Host,
                            Port,
                            ubtrp.Status);
                        break;
            ...
            {
                WriteLog("ESMEConnection : UnBind : ERROR : Host[{0}] Port[{1}] {2}",
                    Host,
                    Port,
                    exception.Message);
            }

            WriteLog("ESMEConnection : Bind : Completed : Host[{0}] Port[{1}] RetValue[{2}]",
                Host,
                Port,
                retValue);
        ...
        private void AddMessageSegmentToCollector(DeliverSm data)
        {
            try
            {
                ...
                var key = data.SourceAddr + data.MessageReferenceNumber;

                lock (Collector)
                {
                    if (Collector.ContainsKey(key)) { userDataControl = Collector[key]; }
                    else
                    {
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : AddMessageSegmentToCollector : ERROR : {0}",
                    exception.ToString());
            }
        }
        ...
        private bool IsLastSegment(DeliverSm data)
        {
            ...
            try
            {
                ...
                lock (Collector) { if (Collector.ContainsKey(key)) userDataControl = Collector[key]; }

                if (userDataControl != null) { if (userDataControl.SegmentsLeftToReceived <= 0) finished = true; }
                else
                {
                    WriteLog(LogEventNotificationTypes.Email,
                        "ESMEConnection : IsLastSegment : ERROR : No User Data Found");
                }
            }
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : IsLastSegment : ERROR : {0}",
                    exception.ToString());
            }
        ...
        private string RetrieveFullMessage(DeliverSm data)
        {
            ...
            try
            {
                ...
                if (userDataControl != null)
                    message = userDataControl.UserData.ShortMessageText(data.DefaultEncoding,
                        data.DataCoding);
                else
                    WriteLog(LogEventNotificationTypes.Email,
                        "ESMEConnection : RetrieveFullMessage : ERROR : No User Data Found");
            }

            catch (Exception exception)
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : RetrieveFullMessage : ERROR : {0}",
                    exception.ToString());
            }
            ...
            out SubmitSmResp submitSmResp)
        {
            ...
            try
            {
                ...
                if (encodeDataCoding == DataCodings.UCS2)
                {
                    ...
                    {
                        WriteLog(LogEventNotificationTypes.Email,
                            "ESMEConnection : SendMessage : WARNING : Truncating UCS2 message to 70 characters.");

                        // The default is Unicode so truncate the message
                        sendMessage = message.Substring(0,
                            70);
                    }
                }
                else
                {
                    ...
                    {
                        WriteLog(LogEventNotificationTypes.Email,
                            "ESMEConnection : SendMessage : WARNING : Truncating Default message to 160 characters.");
                ...
                submitSm = Client.PrepareSubmit(SubmitMode.ShortMessage,
                    serviceType,
                ...
                // Log the send call
                WriteLog("ESMEConnection : SendMessage : Send : Sequence[{0}] Phone[{1}] Status[{2}]",
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : SendMessage : ERROR : {0}",
                    exception.ToString());
        ...
        /// <returns> 0 - Successful / 1 - Failed / 2 - Not Connected </returns>
        public int SendMessageLarge(string phoneNumber,
            string serviceType,
            Ton destinationTon,
            Npi destinationNpi,
            DataCodings submitDataCoding,
            DataCodings encodeDataCoding,
            string message,
            out List<SubmitSm> submitSmList,
            out List<SubmitSmResp> submitSmRespList)
        {
            var retVal = 1;
            ...
            try
            {
                ...
                // Prepare the message, I have made sure there is only ever one message
                // with the trunacting above
                submitSmList = Client.PrepareSubmitLarge(SubmitMode.ShortMessage,
                    serviceType,
                    (byte) Ton.NetworkSpecific,
                    (byte) Npi.Unknown,
                    ShortLongCode,
                ...
                foreach (var submitSmResp in submitSmRespList)
                {
                    // Log the send call
                    WriteLog("ESMEConnection : SendMessage : Send : Sequence[{0}] Phone[{1}] Status[{2}]",
                        submitSmResp.Sequence,
                    ...
                    // Was it successful
                    if (submitSmResp.Status != CommandStatus.ESME_ROK) WriteLog("ESMEConnection : SendMessage : ERROR : Failed For Unknown Reason");
                }
        ...
        public QuerySm SendQuery(string messageId)
        {
            ...
            try
            {
                ...
                // Prepare the query
                querySm = QuerySm.Create(Client.DefaultEncoding,
                    messageId,
                    (byte) Ton.NetworkSpecific,
                    (byte) Npi.Unknown,
                    ShortLongCode);
                ...
                // Send the query
                var querySmResp = Client.Query(querySm);

                // Log the send call
                WriteLog("ESMEConnection : SendQueryThroughSMPP : Send : MessageId[{0}] Sequence[{1}] Status[{2}]",
                    messageId,
                    querySm.Sequence,
                    querySmResp.Status);
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : SendQueryThroughSMPP : ERROR : {0}",
                    exception.ToString());
            }
        ...
        private void ClientEventConnect(object sender,
            bool bSuccess)
        { }
            ...
            try
            {
                if (IsBound)
                {
                    ...
                    if (ConnectionEventHandler != null)
                        ConnectionEventHandler(LogKey,
                            ConnectionEventTypes.Disconnected,
                            string.Format("ESMEConnection : ClientEventDisconnect : WARNING : Host[{0}] Port[{1}] Connection Disconnected",
                                Host,
                                Port));
                }

                // Do we need to try and connect again
                if (Disposed == false) ConnectEvent.Set();
            }

            catch (Exception exception)
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : ClientEventDisconnect : ERROR : {0}",
                    exception.ToString());
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private CommandStatus ClientEventDeliverSm(object sender,
            DeliverSm data)
        {
            WriteLog("ESMEConnection : ClientEventDeliverSm : Started : SegmentNumber[{0}]",
                data.SeqmentNumber);
            ...
            try
            {
                ...
                if (data.SeqmentNumber > 0)
                {
                    ...
                    var logMessage = string.Format("ServiceType[{0}] DestAddr[{1}] SourceAddr[{2}] MessageReferenceNumber[{3}] Sequence[{4}] SeqmentNumber[{5}] TotalSegments[{6}] DataCoding[{7}] MessageText[{8}]",
                        data.ServiceType,
                        data.DestAddr,
                        data.SourceAddr,
                        ...
                        data.TotalSegments,
                        data.DataCoding,
                        data.UserData.ShortMessageText(data.DefaultEncoding,
                            data.DataCoding));

                    WriteLog("ESMEConnection : ClientEventDeliverSm : Info : Partial Message : {0}",
                        logMessage);

                    if (IsLastSegment(data)) message = RetrieveFullMessage(data);
                }
                else
                {
                    // There is only 1 seqment to the outbound message
                    message = data.UserData.ShortMessageText(data.DefaultEncoding,
                        data.DataCoding);
                }

                if (message != null && ReceivedMessageHandler != null)
                    ReceivedMessageHandler(LogKey,
                        data.ServiceType,
                        (Ton) data.SourceTon,
                        (Npi) data.SourceNpi,
                        ShortLongCode,
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : ClientEventDeliverSm : ERROR : {0}",
                    exception.ToString());
            }
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private void ClientEventSubmitSmResp(object sender,
            SubmitSmResp data)
        {
            try
            {
                WriteLog("ESMEConnection : ClientEventSubmitSmResp : Info : Sequence[{0}] Status[{1}]",
                    data.Sequence,
                    data.Status);

                if (SubmitMessageHandler != null)
                    SubmitMessageHandler(LogKey,
                        (int) data.Sequence,
                        data.MessageId);
            }
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : ClientEventSubmitSmResp : ERROR : {0}",
                    exception.ToString());
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private void ClientEventQuerySmResp(object sender,
            QuerySmResp data)
        {
            try
            {
                WriteLog("ESMEConnection : ClientEventQuerySmResp : Info : MessageId[{0}] MessageState[{1}] Status[{2}]",
                    data.MessageId,
                    data.MessageState,
                    data.Status);

                if (QueryMessageHandler != null)
                    QueryMessageHandler(LogKey,
                        (int) data.Sequence,
                        data.MessageId,
                        data.FinalDate,
                        (int) data.MessageState,
                        data.ErrorCode);
            }
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : ClientEventQuerySmResp : ERROR : {0}",
                    exception.ToString());
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private CommandStatus ClientEventEnquireLinkSm(object sender,
            EnquireLinkSm data)
        {
            WriteLog("ESMEConnection : ClientEventEnquireLinkSm : Info : Enquire Link : Command[{0}] Length[{1}] Sequence[{2}] Status[{3}]",
                data.Command,
                data.Length,
                data.Sequence,
                data.Status);
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private void ClientEventEnquireLinkSmResp(object sender,
            EnquireLinkSmResp data)
        {
            WriteLog("ESMEConnection : ClientEventEnquireLinkSmResp : Info : Enquire Link Resp : Command[{0}] Length[{1}] Sequence[{2}] Status[{3}]",
                data.Command,
                data.Length,
                data.Sequence,
                data.Status);
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private void ClientEventGenericNackSm(object sender,
            GenericNackSm data)
        {
            WriteLog("ESMEConnection : ClientEventGenericNackSm : Info : Received Generic Nack  Status[{0}]",
                data.Status);
            ...
            try
            {
                ...
                if (ReceivedGenericNackHandler != null)
                    ReceivedGenericNackHandler(LogKey,
                        (int) data.Sequence);
            }
            ...
            {
                WriteLog(LogEventNotificationTypes.Email,
                    "ESMEConnection : ClientEventGenericNackSm : ERROR : {0}",
                    exception.ToString());
            }
        }
        ...
        /// <param name="ex"></param>
        private void ClientEventError(object sender,
            string comment,
            Exception ex)
        {
            if (ex != null)
                WriteLog("ESMEConnection : ClientEventError : Info : {0} : {1}",
                    comment,
                    ex.Message);
            else
                WriteLog("ESMEConnection : ClientEventError : ERROR : {0}",
                    comment);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="data"></param>
        private CommandStatus ClientEventUnBindSm(object sender,
            UnBindSm data)
        {
            if (ConnectionEventHandler != null)
                ConnectionEventHandler(LogKey,
                    ConnectionEventTypes.UnBound,
                    string.Format("ESMEConnection : ClientEventUnBindSm : WARNING : Host[{0}] Port[{1}] Connection UnBound",
                        Host,
                        Port,
                        ConnectionMode,
                        ConnectionId));
        ...
        /// <param name="details"></param>
        /// <returns> External Id </returns>
        private Guid? ClientEventPduDetails(object send,
            PduDirectionTypes pduDirectionType,
            Header pdu,
            List<PduPropertyDetail> details)
        {
            ...
            if (PduDetailsEventHandler != null)
                externalId = PduDetailsEventHandler(LogKey,
                    pduDirectionType,
                    pdu,
                    details);
