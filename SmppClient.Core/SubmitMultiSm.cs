...
namespace SmppClient.Core
{
    ...
    public class SubmitMultiSm : Header, IPacket, IPduDetails
    {
        ...
        private SubmitMultiSm(DataCodings defaultEncoding) : base(defaultEncoding,
            CommandSet.SubmitMultiSm,
        ...
        private SubmitMultiSm(DataCodings defaultEncoding,
            ...
            byte sourceNpi,
            string sourceAddr,
            List<DestinationAddress> destinationAddresses) : base(defaultEncoding,
            CommandSet.SubmitMultiSm,
            SequenceGenerator.Counter)
        {
            Optional = new TLVCollection();
        ...
        public static SubmitMultiSm Create(DataCodings defaultEncoding,
            string serviceType,
            byte srcTon,
            byte srcNpi,
            string srcAddr,
        ...
        {
            return new SubmitMultiSm(defaultEncoding,
                serviceType,
                srcTon,
                srcNpi,
                srcAddr,
        ...
        public static SubmitMultiSm Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var submitMulti = new SubmitMultiSm(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(submitMulti,
                    ref offset);
                ...
                if (submitMulti.ShortMessageLength > 0)
                    submitMulti.UserDataBuffer = new SmppBuffer(defaultEncoding,
                        buf.ExtractByteArray(ref offset,
                            submitMulti.ShortMessageLength));

                while (offset - start_offset < submitMulti.Length) submitMulti.Optional.Add(buf.ExtractTLV(ref offset));
            }

            catch { submitMulti = null; }
