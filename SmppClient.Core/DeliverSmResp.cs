...
namespace SmppClient.Core
{
    ...
    public class DeliverSmResp : Header, IPacket, IPduDetails
    {
        ...
        public static DeliverSmResp Create(DataCodings defaultEncoding,
            ...
            CommandStatus commandStatus)
        {
            if (deliverSm.SourceAddr == string.Empty || deliverSm.SourceAddr == null) commandStatus = CommandStatus.ESME_RINVSRCADR;
        ...
        public static DeliverSmResp Create(DataCodings defaultEncoding,
            ...
            ref int offset)
        {
            var deliverSmResp = new DeliverSmResp(defaultEncoding);

            try
            {
                var start_offset = offset;

                buf.ExtractHeader(deliverSmResp,
                    ref offset);

                if (deliverSmResp.Length > HEADER_LENGTH)
                {
                    deliverSmResp.MessageId = buf.ExtractCString(ref offset);

                    while (offset - start_offset < deliverSmResp.Length) deliverSmResp.Optional.Add(buf.ExtractTLV(ref offset));
                }
            }

            catch { deliverSmResp = null; }
